...
//borrow transaction branch
$.ajax({
    type: "POST",
    url: "../class/display/display",
    data: {
        key: "display_memberselect_branch"
    }
})
    .done(function (data) {
        var a = JSON.parse(data);
        ...
        $('select[id="borrow_membername"]').html("<option disabled selected></option>" + b).select2({
            placeholder: 'Please select borrower'
        });
    });
    ...
    .done(function (data) {
        ...
        if (data != 0) {
            ...
            $.each(nn, function (x, y) {
                htm += '<li class="todo-list-item" style="color:#555;">\
                        <div class="checkbox">\
