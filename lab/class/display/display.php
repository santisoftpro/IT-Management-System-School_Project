...
class display
{
	...
	public function display_branch($branch_name)
	{
		global $conn;
		$sql = $conn->prepare('SELECT * FROM item WHERE branch_name = ?');
		$sql->execute(array($branch_name));
		$row = $sql->rowCount();
		$fetch = $sql->fetchAll();
	...
	public function display_memberselect_branch()
	{
		global $conn;
		// session_start();
		include '../../branch/session_branch.php';
		// $_SESSION['branch_name'];
		$sql = $conn->prepare("SELECT * FROM member WHERE m_status = ? AND branch_name=? ORDER BY m_fname ASC");
		$sql->execute(array(1, $branch_name));
		$count = $sql->rowCount();
		$fetch = $sql->fetchAll();
		if ($count > 0) {
			foreach ($fetch as $key => $value) {
	...
	public function display_equipment_new_branch()
	{
		global $conn;
		session_start();
		$sql = $conn->prepare('SELECT * FROM item_stock 
									LEFT JOIN item ON item.id = item_stock.item_id
									WHERE item_stock.item_status = ? AND item.branch_name = "Kayonza Branch"');
	...
	public function display_item_borrow_branch()
	{
		global $conn;
		include '../../branch/session_branch.php';
	...
	public function display_room_reserve_branch()
	{
		global $conn;
		include '../../branch/session_branch.php';
		$sql = $conn->prepare('SELECT * FROM room WHERE rm_name != ? AND branch_name=? ORDER BY rm_name ASC');
		$sql->execute(array('room 310', $branch_name));
		$fetch = $sql->fetchAll();
...
switch ($key) {
	...
	case 'display_memberselect';
		...
		break;
	case 'display_memberselect_branch';
		$display->display_memberselect_branch();
		break;
	...
	case 'display_equipment_new';
		...
		break;
	case 'display_equipment_new_branch';
		$display->display_equipment_new_branch();
		break;
	...
	case 'display_item_borrow';
		...
		break;
	case 'display_item_borrow_branch';
		$display->display_item_borrow_branch();
		break;
	...
	case 'display_room_reserve';
		...
		break;
	case 'display_room_reserve_branch';
		$display->display_room_reserve_branch();
		break;
