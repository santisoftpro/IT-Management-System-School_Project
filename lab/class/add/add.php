...
class add
{
	...
	public function add_users($u_fname, $u_username, $u_password, $u_type, $u_branch)
	{
		global $conn;
		...
		$count = $sql->rowCount();
		if ($count <= 0) {
			$que = $conn->prepare('INSERT INTO user	(name,username,password,type,branches) VALUES(?,?,?,?,?);
										INSERT INTO history_logs(description,table_name,user_id,user_type) VALUES(?,?,?,?)');
			$que->execute(array($u_fname, $u_username, $u_password, $u_type, $u_branch, $h_desc, $h_tbl, $sessionid, $sessiontype));
			$row = $que->rowCount();
			if ($row > 0) {
				echo "1";
			} else {
